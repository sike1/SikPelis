...
exports.nuevaPelicula=async(req,res,next)=>{
  ...
  req.flash("exito", "Pelicula insertada");
...
exports.mostrarPeliculas=async(req,res)=>{
    ...
    const pelicula = await Peliculas.findOne({where:{slug:req.params.slug}})
...
exports.peliculaFav=async(req,res)=>{
    ...
    if(accion === "favorito"){
        //agregamos a fav
        Usuarios.update(
            {pelisFavs: Sequelize.fn("array_append",Sequelize.col("pelisFavs"),pelicula.id)},
        ...
        //eliminamos de fav
        Usuarios.update(
            {pelisFavs: Sequelize.fn("array_remove",Sequelize.col("pelisFavs"),pelicula.id)},
            {where:{id: req.usuario.id}})
         //mensaje
